...
struct TweetCounts(HashMap<String, Vec<i32>>);
...
impl TweetCounts {
    ...
    fn new() -> Self {
        TweetCounts(HashMap::new())
    }
    
    fn record_tweet(&mut self, tweet_name: String, time: i32) {
        self.0.entry(tweet_name).or_insert(Vec::new()).push(time);
    }
    ...
    fn get_tweet_counts_per_frequency(&self, freq: String, tweet_name: String, start_time: i32, end_time: i32) -> Vec<i32> {
        ...
        let interval = end_time - start_time + 1;
        let mut buckets = interval / freq;
        if interval % freq > 0 {buckets += 1;}
        let mut result = vec![0; buckets as usize];
        for time in self.0.get(&tweet_name).unwrap_or(&vec![]) {
            if *time >= start_time && *time <= end_time {
                result[((*time - start_time) / freq)as usize] += 1;
            }
        }
        result
    }
